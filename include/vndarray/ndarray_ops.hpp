...
inline void require_same_extents(View<T, Rank> a, View<U, Rank> b, View<V, Rank> c) noexcept
{
    if constexpr (runtime_shape_check) {
        require_contract(same_extents(a, b));
...
template<typename T, index_t Rank> inline ByteEnvelope byte_envelope(View<T, Rank> v) noexcept
{
...
template<typename T, index_t Rank> inline index_t byte_lattice_step(View<T, Rank> v) noexcept
{
    using E   = std::remove_const_t<T>;
    index_t g = 0;
...
inline void for_each_offset3(View<T, Rank> a, View<U, Rank> b, View<V, Rank> c, F&& f) noexcept
{
    if (a.empty()) {
        return;
    }
...
template<typename T>
inline void axpby_contiguous_noalias(index_t n, T* VEHKKO_NDARRAY_RESTRICT dst, T alpha, const T* x,
                                     T beta, const T* y) noexcept
{
    for (index_t i = 0; i < n; ++i) {
...
template<typename T> using abs_result_t = typename abs_result<std::remove_const_t<T>>::type;
...
template<typename T, index_t Rank> inline void fill_inplace(View<T, Rank> dst, T value) noexcept
{
...
template<typename T, index_t Rank> inline void scale_inplace(View<T, Rank> dst, T alpha) noexcept
{
...
template<typename T, index_t Rank> inline void shift_inplace(View<T, Rank> dst, T beta) noexcept
{
...
inline void copy_to(View<T, Rank> dst, View<S, Rank> src) noexcept(!runtime_alias_dispatch)
{
    ...
    if constexpr (!runtime_alias_dispatch) {
        if (is_row_major_contiguous(dst) && is_row_major_contiguous(src)) {
            std::memmove(static_cast<void*>(dst.data()),
...
inline void add_to_noalias(View<T, Rank> dst, View<X, Rank> x, View<Y, Rank> y) noexcept
{
    ...
    static_assert(
        std::is_same_v<T, std::remove_const_t<X>> && std::is_same_v<T, std::remove_const_t<Y>>,
    ...
    if (is_row_major_contiguous(dst) && is_row_major_contiguous(x) && is_row_major_contiguous(y)) {
                   ...
                   View<Y, Rank> y) noexcept(!runtime_alias_dispatch)
{
    ...
    if constexpr (runtime_alias_dispatch) {
        ...
        if (rx == detail::AliasRelation::may_overlap || ry == detail::AliasRelation::may_overlap) {
            auto temporary = detail::make_array_like<T>(dst);
...
inline void add_inplace_noalias(View<T, Rank> dst, View<X, Rank> x) noexcept
{
    ...
    const T* xp = x.data();
    detail::for_each_offset2(
        dst, x, [&](index_t doff, index_t xoff) noexcept { d[doff] += xp[xoff]; });
}
...
inline void add_inplace(View<T, Rank> dst, View<X, Rank> x) noexcept(!runtime_alias_dispatch)
{
    ...
    static_assert(std::is_same_v<T, std::remove_const_t<X>>, "add_inplace: value types must match");
...
inline void sub_inplace(View<T, Rank> dst, View<X, Rank> x) noexcept(!runtime_alias_dispatch)
{
    ...
    static_assert(std::is_same_v<T, std::remove_const_t<X>>, "sub_inplace: value types must match");
...
inline void mul_inplace(View<T, Rank> dst, View<X, Rank> x) noexcept(!runtime_alias_dispatch)
{
    ...
    static_assert(std::is_same_v<T, std::remove_const_t<X>>, "mul_inplace: value types must match");
...
template<typename T, index_t Rank> inline std::remove_const_t<T> sum(View<T, Rank> x) noexcept
{
...
template<typename T, index_t Rank> inline detail::abs_result_t<T> max_abs(View<T, Rank> x) noexcept
{
...
template<typename T, index_t Rank> inline std::remove_const_t<T> min_value(View<T, Rank> x) noexcept
{
    using R = std::remove_const_t<T>;
    if constexpr (runtime_shape_check) {
    ...
    const T* p      = x.data();
    R        result = p[0];
    detail::for_each_offset(x, [&](index_t o) noexcept { result = p[o] < result ? p[o] : result; });
    return result;
}

template<typename T, index_t Rank> inline std::remove_const_t<T> max_value(View<T, Rank> x) noexcept
{
    using R = std::remove_const_t<T>;
    if constexpr (runtime_shape_check) {
        require_contract(!x.empty());
    ...
    const T* p      = x.data();
    R        result = p[0];
    detail::for_each_offset(x, [&](index_t o) noexcept { result = result < p[o] ? p[o] : result; });
    return result;
}
